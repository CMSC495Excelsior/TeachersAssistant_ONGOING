...
public class TA_ReportsPage {
    ...
    public static Scene startReportsPage(int courseID_Arg){ 
        // Test attendance and assignment maps
        //TreeMap<Integer, TreeMap<Integer, Float>> test = TA_SQLFunctions.createAssignmentGradesList();
        //TreeMap<Integer, TreeMap<Integer, TreeMap<LocalDate, Integer>>> test2 = TA_SQLFunctions.createStudentAttendanceMap();
    ...
    private static BorderPane makeBorderPane(){
        ...
        downloadExcelDocButton = new Button("Download Excel Dump");
        downloadExcelDocButton.setAlignment(Pos.CENTER);
        downloadExcelDocButton.setStyle("-fx-background-color: #ff0000; -fx-font-weight: bold;");
    ...
    private static GridPane makeGridPane(){
        ...
        sideButtonBox = TA_ConvenienceMethods.createVBox(sideButtonBox, Pos.CENTER, Double.MAX_VALUE,
                10, null, 5, 5, 5, 5, selectAllStudents, viewStudents_CurrentClass, viewStudents_ALL, downloadExcelDocButton);
    ...
    private static void setActionListeners(){
        ...
        downloadExcelDocButton.setOnAction(event -> {
            ...
            if(TA_AlertPage.alert("Download File", "You are about to download a LARGE file.\nWould you like to continue?", Color.CORAL, true)){
                TA_ExcelFunctions.batchDownload();
                TA_AlertPage.alert("Download Success", "Your Excel dump has been downloaded at:\n\tC:\\Users\\Public\\Documents\\ExcelDump_" + LocalDate.now() + ".xlsx", Color.CORAL);
            }
            else
                TA_AlertPage.alert("Download Refusal", "You have opted not to download the Excel file.", Color.CORAL);
        });
    }
