...
public class TA_SQLFunctions {
                    ...
                    for(int cellNumber = 0; cellNumber < row.getPhysicalNumberOfCells(); cellNumber++){
                        ...
                        try{
                            ...
                            TA_AlertPage.alert("Excel Parsing Error", "Error with parsing the selected Excel file."
                                    ...
                                    + "\nFor more information, please review the User Guide", Color.web("#eaece5"));
                            e.printStackTrace();
                        }
                    }
    ...
    // Method to return a map of StudentIDs mapped to attendance values
    public static TreeMap<Integer, Integer> getStudentAttendance(int courseID_ARG, TreeMap<Integer, ToggleGroup> studentIDs_ARG, LocalDate selectedDate){
        // Create Map to return
        TreeMap<Integer, Integer> studentAttendanceMap = new TreeMap<>();
        
        // Begin creating connection
        try{
            // Create connection
            Connection con = connectDB();
            
            // Create statement
            PreparedStatement getAttendance = con.prepareStatement("SELECT attendance FROM StudentAttendance WHERE "
                    + "student_ID = ? "
                    + "AND course_ID = ? "
                    + "AND attendanceDate = ?");
            
            // Iterate through studentIDs_ARG list
            for(Map.Entry<Integer, ToggleGroup> entry : studentIDs_ARG.entrySet()){
                // Bind updated statement parameters
                getAttendance.setInt(1, entry.getKey());
                getAttendance.setInt(2, courseID_ARG);
                getAttendance.setDate(3, java.sql.Date.valueOf(selectedDate));
                
                // Execute statement and save results to map
                ResultSet attendanceSet = getAttendance.executeQuery();
                
                while(attendanceSet.next()){
                    // Save to TreeMap
                    if(attendanceSet.getInt("attendance") == -1){
                        // Return
                        return null;
                    }
                    else{
                        // Insert records into Map
                        studentAttendanceMap.put(entry.getKey(), attendanceSet.getInt("attendance"));
                    }
                }
                // Close set
                attendanceSet.close();
            }
            // Close Stmt and Conneciton
            getAttendance.close();
            con.close();
    ...
    // Method to return a map of StudentIDs mapped to attendance values
    public static TreeMap<Integer, Float> getStudentAssignmentGrades(TreeMap<Integer, TextField> studentIDs_ARG, int assignmentID_ARG){
        // Create Map to return
        TreeMap<Integer, Float> studentGradesMap = new TreeMap<>();
        ...
        try{
            ...
            // Create statement
            PreparedStatement getGrades = con.prepareStatement("SELECT grade FROM StudentAssignmentGrades WHERE "
                    + "student_ID = ? "
                    + "AND assignment_ID = ? ");
            
            // Iterate through studentIDs_ARG list
            for(Map.Entry<Integer, TextField> entry : studentIDs_ARG.entrySet()){
                // Bind updated statement parameters
                getGrades.setInt(1, entry.getKey());
                getGrades.setInt(2, assignmentID_ARG);
                
                // Execute statement and save results to map
                ResultSet gradesSet = getGrades.executeQuery();
                
                while(gradesSet.next()){
                    // Save to TreeMap
                    if(gradesSet.getFloat("grade") < 0){
                        // Return
                        return null;
                    }
                    else{
                        // Insert records into Map
                        studentGradesMap.put(entry.getKey(), gradesSet.getFloat("grade"));
                    }
                }
                // Close set
                gradesSet.close();
        ...
        try{
            ...
            while(assignmentIDSet.next()){
                ...
                while(gradesSet.next()){
                    ...
                    // The default grade for assignment grades is -1.0, so check for grades less than 0
                    if(tempGrade < 0){
    ...
    private static void calculateOverallCourseGrade(int course_ID){
        ...
        try{
            ...
            while(studentIDSet.next()){
                ...
                updateStudentCourseGrade(tempStudentID, course_ID, finalCourseGrade*100);
    ...
    private static void updateStudentCourseGrade(int studentID, int course_ID, float finalCourseGrade){
        ...
        try{
            ...
            System.out.println("Final Course Grade for StudentID: " + studentID + " in courseID: " + course_ID + " is: " + finalCourseGrade);
        }
        catch(Exception e){
            // Print stack trace
            e.printStackTrace();
            ...
            while(studentIDSet.next()){
                ...
                while(courseInformationSet.next()){
                    ...
                    while(studentCourseGradeSet.next()){
                        // Save tempGrade
                        float courseDigit = studentCourseGradeSet.getFloat("grade");
